...
class spreadsheet {
    ...
    public function NoLogin() { $this->no_auth = "true"; }
	...
	public function addToResourceId($spreadsheet_key, $data) {
		$url = "https://spreadsheets.google.com/feeds/list/$spreadsheet_key/od7/private/full";
        iprint("PostUrl() = [$url]");
		$headers = array(
		...
		if($columnIDs) {
	        ...
	        if ($response === false && ($status != 200 || $status != 201)) { 
		        error("Error: html return code = $status<br/> response=$response ".curl_errno($curl));    
		        die("Curl Error http status code=$status" . curl_error($curl) . "<br/>" . $response);    
	        }
	...
	public function add($data) {
		if(!empty($this->token)) {
			$url = $this->getPostUrl();
            iprint("add original PostUrl() = [$url]");
			if(!empty($url)) {
				$headers = array(
					"Content-Type: application/atom+xml",
					"Authorization: GoogleLogin auth=" . $this->token,
